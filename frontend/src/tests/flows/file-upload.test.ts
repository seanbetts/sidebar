...
describe('file upload flow', () => {
  it('uploads a file and requests metadata', async () => {
    ingestionAPI.upload.mockImplementation(async (_file: File, onProgress?: (p: number) => void) => {
      onProgress?.(50);
      return { file_id: 'file-123' };
    });
    ingestionAPI.get.mockResolvedValue({
      file: { id: 'file-123', filename_original: 'test.txt', mime_original: 'text/plain', size_bytes: 3 },
