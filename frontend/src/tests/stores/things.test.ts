...
describe('thingsStore', () => {
  ...
  it('removes completed tasks and updates counts', async () => {
    thingsAPI.list.mockResolvedValue({
      scope: 'today',
      tasks: [
        { id: 'task-1', title: 'Task', status: 'open', areaId: null, projectId: null }
      ],
      areas: [],
      projects: []
    });
  ...
  it('moves tasks between today and upcoming caches', async () => {
    ...
    thingsAPI.apply.mockResolvedValue(undefined);
    thingsAPI.counts.mockResolvedValue({ counts: { inbox: 0, today: 0, upcoming: 1 }, areas: [], projects: [] });
  ...
  it('updates notes locally after applying', async () => {
    thingsAPI.list.mockResolvedValue({
      scope: 'today',
      tasks: [{ id: 'task-4', title: 'Note', status: 'open', notes: '', areaId: null, projectId: null }],
      areas: [],
      projects: []
    });
    thingsAPI.apply.mockResolvedValue(undefined);
