...
function createIngestionStore() {
	...
	return {
		...
		updateFileFields(fileId: string, patch: Partial<IngestionListItem['file']>) {
			update((state) => {
				const nextItems = state.items.map((item) =>
					item.file.id === fileId ? { ...item, file: { ...item.file, ...patch } } : item
				);
				persistCache(nextItems, state.localUploads);
				return {
					...state,
		...
		updateJob(fileId: string, patch: Partial<IngestionListItem['job']>) {
			update((state) => {
				const nextItems = state.items.map((item) =>
					item.file.id === fileId ? { ...item, job: { ...item.job, ...patch } } : item
				);
				persistCache(nextItems, state.localUploads);
				return {
					...state,
		...
		async revalidateInBackground() {
			try {
				...
				logError('Failed to revalidate ingestion status', error, {
					scope: 'ingestionStore.revalidateInBackground'
				});
			}
		},
