...
export function insertNoteIntoFolder(
	nodes: FileNode[],
	noteNode: FileNode,
	folderPath: string
): FileNode[] {
...
export function ensureFolderPath(
	...
	const ensureRecursive = (items: FileNode[], remaining: string[], prefix: string): FileNode[] => {
		...
		const nextItems = items.map((item) => {
			...
			const updatedChildren = rest.length
				? ensureRecursive(item.children, rest, currentPath)
				: item.children;
...
export function updateWorkspaceNodePaths(
	node: FileNode,
	oldPrefix: string,
	newPrefix: string,
	renameTo?: string
