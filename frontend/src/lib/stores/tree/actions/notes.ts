...
export function createNotesActions(context: TreeStoreContext) {
	const addNoteNode = (payload: {
		...
		context.update((state) => {
			...
			const existingIndex = current.findIndex(
				(node) => node.type === 'file' && node.path === payload.id
			);
			if (existingIndex >= 0) {
	...
	const setNotePinned = (noteId: string, pinned: boolean) => {
		context.update((state) => {
			...
			const result = updateNoteInTree(tree.children, noteId, (node) => ({
				...
				pinned,
				...(pinned
					? { pinned_order: node.pinned_order ?? nextPinnedOrder }
					: { pinned_order: null }),
	...
	const addFolderNode = (path: string) => {
		context.update((state) => {
			...
			const updatedChildren = ensureFolderPath(tree.children || [], normalized, {
				folderMarker: true
			});
	...
	const renameFolderNode = (oldPath: string, newName: string) => {
		context.update((state) => {
			...
			const updatedExpandedPaths = updateExpandedPathsForFolder(
				tree.expandedPaths,
				normalizedOld,
				newPath
			);
