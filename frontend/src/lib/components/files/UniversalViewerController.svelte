<script lang="ts">
	...
	$: showMarkdownToggle = hasMarkdown && !isSpreadsheet && !isAudio && !isVideo && !isMarkdownOnly;
	...
	let PdfViewerComponent: typeof import('$lib/components/files/PdfViewer.svelte').default | null =
		null;
	let isCopied = false;
	...
	function stripFrontmatter(text: string): string {
		...
		if (separatorIndex > 0) {
			...
			const allMetadata = metadataLines.every(
