<script lang="ts">
	...
	$: headerRow = headerRowIndex !== null && rawRows[headerRowIndex] ? rawRows[headerRowIndex] : [];
	...
	$: rowsToShow = Math.min(totalRows, baseRowsPerView * Math.max(1, loadMorePages + 1));
	...
	{:else}
		<div class="spreadsheet-controls">
			...
			<div class="sheet-filter">
				<input type="text" placeholder="Filter rows…" bind:value={filterText} />
		...
		<div class="sheet-pagination">
			...
			{#if rowsToShow < totalRows}
				<button class="sheet-page-button" onclick={() => (loadMorePages += 1)}> Load more </button>
			{/if}
		</div>
