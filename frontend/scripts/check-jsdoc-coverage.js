...
function analyzeFile(filePath) {
  ...
  if (fileStats.functions.total > 0 || fileStats.classes.total > 0 || fileStats.methods.total > 0) {
    stats.totalFiles++;
    if (fileStats.functions.missing.length > 0 ||
...
function main() {
  ...
  const documentedItems = stats.documentedFunctions + stats.documentedClasses + stats.documentedMethods;
  const overallCoverage = calculateCoverage(documentedItems, totalItems);

  console.log('| Category   | Total | Documented | Coverage |');
  console.log('|------------|-------|------------|----------|');
  console.log(`| Functions  | ${stats.totalFunctions.toString().padStart(5)} | ${stats.documentedFunctions.toString().padStart(10)} | ${functionCoverage.padStart(7)}% |`);
  console.log(`| Classes    | ${stats.totalClasses.toString().padStart(5)} | ${stats.documentedClasses.toString().padStart(10)} | ${classCoverage.padStart(7)}% |`);
  console.log(`| Methods    | ${stats.totalMethods.toString().padStart(5)} | ${stats.documentedMethods.toString().padStart(10)} | ${methodCoverage.padStart(7)}% |`);
  console.log(`| **TOTAL**  | ${totalItems.toString().padStart(5)} | ${documentedItems.toString().padStart(10)} | ${overallCoverage.padStart(7)}% |`);
  console.log();
  ...
  if (stats.filesWithIssues > 0) {
    ...
    const sorted = stats.fileDetails
      .filter(f => f.functions.missing.length + f.classes.missing.length + f.methods.missing.length > 0)
      .sort((a, b) => {
        const aMissing = a.functions.missing.length + a.classes.missing.length + a.methods.missing.length;
        const bMissing = b.functions.missing.length + b.classes.missing.length + b.methods.missing.length;
        return bMissing - aMissing;
      })
      .slice(0, 10);

    for (const file of sorted) {
      const totalMissing = file.functions.missing.length + file.classes.missing.length + file.methods.missing.length;
      console.log(`${file.path} (${totalMissing} missing)`);

      if (file.classes.missing.length > 0) {
        console.log(`  Classes: ${file.classes.missing.map(c => `${c.name}:${c.line}`).join(', ')}`);
      }
      if (file.functions.missing.length > 0) {
        console.log(`  Functions: ${file.functions.missing.map(f => `${f.name}:${f.line}`).join(', ')}`);
      }
      if (file.methods.missing.length > 0) {
        console.log(`  Methods: ${file.methods.missing.map(m => `${m.name}:${m.line}`).join(', ')}`);
      }
      console.log();
    }
  }
