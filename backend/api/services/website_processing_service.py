...
class WebsiteProcessingService:
    ...
    @staticmethod
    def get_job(db: Session, user_id: str, job_id: uuid.UUID) -> Optional[WebsiteProcessingJob]:
    ...
    def update_job(
        ...
        *,
        ...
        job = db.query(WebsiteProcessingJob).filter(WebsiteProcessingJob.id == job_id).first()
        if not job:
            return
        job.status = status
