...
def check_ffmpeg() -> bool:
    ...
    try:
        ...
        if missing_codecs:
            print(f"Warning: ffmpeg is missing required codecs: {', '.join(missing_codecs)}")
            return False
...
def verify_output(filepath: str) -> bool:
    ...
    try:
        ...
        if has_video:
            ...
            print(f"  Video stream: {v_stream.get('codec_name', 'unknown')} "
        ...
        if has_audio:
            ...
            print(f"  Audio stream: {a_stream.get('codec_name', 'unknown')} "
...
def progress_hook(d):
    ...
    if d['status'] == 'downloading':
        if 'fragment_index' in d and 'fragment_count' in d:
            print(f"\r  Downloading fragment {d['fragment_index']}/{d['fragment_count']} "
                  f"({d.get('_percent_str', 'N/A')})", end='', flush=True)
    ...
    *,
    ...
    try:
        ...
        if "Unavailable video" in error_msg:
            raise ValueError("Video is unavailable (private, age-restricted, or removed)") from e
