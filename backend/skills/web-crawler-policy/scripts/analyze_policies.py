...
try:
    sys.path.insert(
        0, str(Path(__file__).parent.parent.parent / "subdomain-discover" / "scripts")
    )
...
class RobotsAnalyzer:
    ...
    def _is_valid_llms_txt(self, content: str) -> bool:
        ...
        # Reject HTML content
        if any(
            html_indicator in content
            for html_indicator in [
                "<!doctype",
                "<html",
                "<head>",
                "<body>",
        ...
        if ("# " in content or "- " in content or "## " in content) and not any(
            tag in content for tag in ["<html", "<head", "<body", "<script"]
        ):
            return True

        # If it's short plain text without HTML tags, might be valid
        if len(content) < 500 and not any(
            tag in content for tag in ["<", ">", "{", "}"]
        ):
            return True
    ...
    def parse_robots_txt(self, content: str) -> dict[str, dict[str, list[str]]]:
        ...
        for line in content.split("\n"):
            ...
            if line.lower().startswith("user-agent:"):
                ...
                if not current_user_agents or any(
                    ua in rules and (rules[ua]["allow"] or rules[ua]["disallow"])
    ...
    def save_robots_txt(
        self, domain: str, content: str, file_type: str = "robots"
    ) -> Path | None:
...
"""
    ...
    async def analyze_domain_robots(
        self, session: aiohttp.ClientSession, domain: str
    ) -> dict:
        ...
        if robots_content:
            ...
            result.update(
                {"robots_found": True, "rules": rules, "robots_content": robots_content}
            )
            if saved_path:
                result["robots_saved_to"] = str(saved_path)
        else:
            result.update({"robots_found": False, "rules": {}})
        ...
        if self.check_llms:
            if llms_content:
                ...
                result.update({"llms_found": False, "llms_rules": {}})
    ...
    def _consolidate_after_analysis(
        ...
        for domain, data in robots_data.items():
            ...
            if base_data and www_data:
                ...
                base_content = base_data.get("robots_found", False) or base_data.get(
                    "llms_found", False
                )
                www_content = www_data.get("robots_found", False) or www_data.get(
                    "llms_found", False
                )
...
class CrawlerPermissionAnalyzer:
    ...
    def __init__(self, robots_data: dict[str, dict], include_llms: bool = True):
        ...
        self.standard_llm_crawlers = {
            "gptbot": {
                "description": "OpenAI GPT training and research",
                "provider": "OpenAI",
            },
            "oai-searchbot": {
                "description": "OpenAI search and indexing",
                "provider": "OpenAI",
            },
            "chatgpt-user": {
                "description": "OpenAI ChatGPT user queries",
                "provider": "OpenAI",
            },
            "google-extended": {
                "description": "Google AI training (Bard, Gemini)",
                "provider": "Google",
            },
            "claudebot": {
                "description": "Anthropic Claude training and research",
                "provider": "Anthropic",
            },
            "claude-web": {
            ...
            "ccbot": {
                "description": "Common Crawl (used by many AI companies)",
                "provider": "Common Crawl",
            },
            "meta-externalagent": {
                "description": "Meta AI training and research",
                "provider": "Meta",
            },
            "perplexitybot": {
                "description": "Perplexity AI search and training",
                "provider": "Perplexity",
            },
            "bytespider": {
                "description": "ByteDance AI training",
                "provider": "ByteDance",
            },
        ...
        self.dual_purpose_crawlers = {
            "googlebot": {
                "description": "Google Search & AI Training",
                "provider": "Google",
            },
            "bingbot": {
                "description": "Bing Search & AI Training",
                "provider": "Microsoft",
            },
            "twitterbot": {
                "description": "Twitter Cards & AI Training",
                "provider": "X/Twitter",
            },
            "facebookexternalhit": {
                "description": "Facebook Sharing & AI Training",
                "provider": "Meta",
            },
        }
        ...
        for domain_data in self.robots_data.values():
            if domain_data.get("robots_found"):
                for crawler in domain_data.get("rules", {}).keys():
                    if not self.is_llm_crawler(
                        crawler
                    ) and not self.is_dual_purpose_crawler(crawler):
    ...
    def get_additional_llm_crawlers(self) -> set[str]:
        ...
        for domain_data in self.robots_data.values():
            if domain_data.get("robots_found"):
                for crawler in domain_data.get("rules", {}).keys():
                    if (
                        self.is_llm_crawler(crawler)
                        and crawler.lower() not in self.standard_llm_crawlers
                    ):
    ...
    def analyze_llms_files(self) -> dict[str, dict]:
        ...
        for domain, domain_data in self.robots_data.items():
            if domain_data.get("llms_found"):
                ...
                llms_analysis[domain] = {"found": False, "rules": {}, "raw_content": ""}
    ...
    def analyze_permissions(self) -> dict[str, Any]:
        ...
        results = {
            "robots_analysis": {"traditional": [], "llm": [], "dual_purpose": []},
            "llms_analysis": self.analyze_llms_files() if self.include_llms else {},
            "summary": {
                "total_domains": len(self.robots_data),
                "domains_with_robots": sum(
                    1 for data in self.robots_data.values() if data.get("robots_found")
                ),
                "domains_with_llms": sum(
                    1 for data in self.robots_data.values() if data.get("llms_found")
                ),
                "traditional_crawlers_found": len(traditional_crawlers),
                "llm_crawlers_found": len(self.standard_llm_crawlers)
                + len(found_additional_llm_crawlers),
        ...
        all_llm_crawlers = (
            set(self.standard_llm_crawlers.keys()) | found_additional_llm_crawlers
        )
        for crawler in all_llm_crawlers:
            crawler_data = self._analyze_single_crawler(crawler, "llm")
            crawler_info = self.standard_llm_crawlers.get(
                crawler, {"description": "AI/LLM crawler", "provider": "Unknown"}
            )
            crawler_data["description"] = (
                crawler_info["description"]
                if isinstance(crawler_info, dict)
                else crawler_info
            )
            crawler_data["provider"] = (
                crawler_info.get("provider", "Unknown")
                if isinstance(crawler_info, dict)
                else "Unknown"
            )
        ...
        for crawler in found_dual_purpose_crawlers:
            ...
            crawler_info = self.dual_purpose_crawlers.get(
                crawler.lower(),
                {"description": "Dual-purpose crawler", "provider": "Unknown"},
            )
            crawler_data["description"] = (
                crawler_info["description"]
                if isinstance(crawler_info, dict)
    ...
    def _analyze_single_crawler(self, crawler: str, crawler_type: str) -> dict:
        ...
        crawler_data = {"crawler": crawler, "type": crawler_type, "domains": {}}
...
def create_output_summary(permission_data: dict, domains: set[str]) -> str:
    ...
    output.append(
        f"LLM files discovered: {'Yes' if summary.get('llm_files_found') else 'No'}"
    )
    output.append("=" * 60)
...
def create_output_table(
    permission_data: dict, domains: set[str], main_domain: str = ""
) -> str:
    ...
    sorted_crawlers = sorted(
        llm_crawlers, key=lambda x: (x.get("provider", ""), x["crawler"])
    )

    for crawler_info in sorted_crawlers:
        row = [
            crawler_info.get("provider", ""),
            crawler_info["crawler"],
            crawler_info.get("description", ""),
        ]
        for domain in ordered_domains:
            status = crawler_info["domains"].get(domain, "No robots.txt")
            row.append(status)
    ...
    if dual_purpose_crawlers:
        ...
        sorted_crawlers = sorted(
            dual_purpose_crawlers, key=lambda x: (x.get("provider", ""), x["crawler"])
        )
    ...
    if llms_analysis:
        ...
        domains_with_llms = [
            domain for domain, data in llms_analysis.items() if data.get("found")
        ]
        ...
        if domains_with_llms:
            ...
            if all_directives:
                ...
                for directive in sorted(all_directives):
                    row = [directive]
                    for domain in ordered_domains:
                        if domain in llms_analysis and llms_analysis[domain].get(
                            "found"
                        ):
...
def export_to_csv(
    ...
    with open(output_file, "w", newline="", encoding="utf-8") as csvfile:
        ...
        dual_purpose_crawlers = robots_analysis.get("dual_purpose", [])
        sorted_dual_purpose_crawlers = sorted(
            dual_purpose_crawlers, key=lambda x: (x.get("provider", ""), x["crawler"])
        )
...
Requirements:
  - OPENAI_API_KEY for LLM reports (set via Doppler or environment variable)
    ...
    domain_group.add_argument(
        "domain", nargs="?", help="Target domain to analyze (discovers subdomains)"
    )
    domain_group.add_argument(
        "--domains", nargs="+", help="Specific domains to analyze (skips discovery)"
    )

    # Discovery options
    parser.add_argument(
        "--no-discover", action="store_true", help="Skip subdomain discovery"
    )
    ...
    parser.add_argument(
        "--no-llms", action="store_true", help="Skip checking llms.txt files"
    )
    parser.add_argument(
        "--save-robots",
        action="store_true",
        help="Save robots.txt and llms.txt files to disk",
    )
    parser.add_argument(
        "--output-dir",
        default=DEFAULT_OUTPUT_DIR,
        help=f"R2 output folder (default: {DEFAULT_OUTPUT_DIR})",
    )
    ...
    parser.add_argument(
        "--no-table", action="store_true", help="Skip console table output"
    )
    parser.add_argument(
        "--all", action="store_true", help="Save all output formats with default names"
    )
    ...
    parser.add_argument(
        "--report",
        action="store_true",
        help="Generate LLM-powered marketing intelligence report",
    )
    parser.add_argument(
        "--llm-model", default="gpt-4o", help="LLM model to use (default: gpt-4o)"
    )
    parser.add_argument(
        "--llm-api-key", help="OpenAI API key (or use OPENAI_API_KEY from Doppler/env)"
    )
    ...
    if args.report and not LLM_AVAILABLE:
        print(
            "Error: --report requires llm_report_generator.py module", file=sys.stderr
        )
        sys.exit(1)
    ...
    try:
        ...
        if args.domains:
            ...
            if not SUBDOMAIN_DISCOVERY_AVAILABLE:
                print(
                    "Error: Subdomain discovery not available. Use --domains or --no-discover",
                    file=sys.stderr,
                )
                sys.exit(1)
            ...
            if args.wordlist:
                try:
                    ...
                    print(
                        f"Error: Wordlist file '{args.wordlist}' not found",
                        file=sys.stderr,
                    )
                    sys.exit(1)
            ...
            scanner = SubdomainScanner(
                main_domain, timeout=args.timeout, dns_timeout=args.dns_timeout
            )
        ...
        if args.save_robots:
            robots_saved = sum(
                1 for data in robots_data.values() if data.get("robots_saved_to")
            )
            llms_saved = (
                sum(1 for data in robots_data.values() if data.get("llms_saved_to"))
                if not args.no_llms
                else 0
            )
            if not args.no_llms and llms_saved > 0:
                print(
                    f"Saved {robots_saved} robots.txt and {llms_saved} llms.txt files to disk."
                )
            else:
                print(f"Saved {robots_saved} robots.txt files to disk.")

        # Analyze crawler permissions
        permission_analyzer = CrawlerPermissionAnalyzer(
            robots_data, include_llms=not args.no_llms
        )
        ...
        if args.json_output:
            ...
            if not os.path.dirname(args.json_output):
                print(
                    f"Raw data exported to: {r2_base}/{main_domain}/{args.json_output}"
                )
            else:
        ...
        if args.report:
            ...
            try:
                ...
                analysis_result = await report_generator.generate_report_analysis(
                    full_scan_data
                )

                # Generate markdown report
                markdown_report = generate_markdown_report(
                    analysis_result, full_scan_data
                )
                ...
                if not os.path.dirname(report_output_path):
                    ...
                    analysis_json_path = (
                        str(domain_folder / Path(report_output_path).stem)
                        + "_analysis.json"
                    )
                    with open(analysis_json_path, "w", encoding="utf-8") as f:
                        json.dump(analysis_result, f, indent=2)
                    print(
                        f"Analysis data saved to: {r2_base}/{main_domain}/{Path(analysis_json_path).name}"
                    )
                ...
                if not os.path.dirname(report_output_path):
                    print(
                        f"Intelligence report generated: {r2_base}/{main_domain}/{Path(report_output_path).name}"
                    )
                else:
        ...
        if (
            create_ingested_file is None
            or write_derivative is None
            or write_ai_markdown is None
        ):
        ...
        summary_markdown = _build_summary_markdown(
            main_domain, permission_data, domains_to_analyze
        )
