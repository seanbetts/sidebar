...
def test_create_and_list_conversations(db_session):
    conversation = ConversationService.create_conversation(db_session, "user-1", "New Chat")
...
def test_add_message_updates_metadata(db_session):
    ...
    updated = ConversationService.add_message(db_session, "user-1", conversation.id, payload)
...
def test_search_conversations(db_session):
    title_match = ConversationService.create_conversation(db_session, "user-1", "Alpha Chat")
    message_match = ConversationService.create_conversation(db_session, "user-1", "Other Chat")
    ...
    title_results = ConversationService.search_conversations(db_session, "user-1", "Alpha", limit=10)
    message_results = ConversationService.search_conversations(db_session, "user-1", "Delta", limit=10)
...
def test_archive_conversation_excluded_from_list(db_session):
    conversation = ConversationService.create_conversation(db_session, "user-1", "Archive Me")
